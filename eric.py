...
class editDecks(tk.Frame):
    def __init__(self, master,controller):
        ...
        self.emptyDeck = tk.Message(self)
        self.termGUI = tk.Message(self, font="Fireye 14 bold", fg="black")
        self.defGUI = tk.Message(self, font="Fireye 14 bold", fg="black")
...
#
    ...
    def edit_card(self, deck_name):
        ...
        # User needs to click save deck
        self.button["command"] = lambda : self.saveContents(deck_name, self.cardFront, self.backCard)
        self.button.pack(side="top")

    def saveContents(self, deck_name, cardFront, backCard):
        global mainController
        global mainDeckName
        global mainFileSys

        # This will check if the user actually entered a term and definition before it creates the new card.  The first three cases handle both or either being empty.
        if len(self.cardFront) == 0 and len(self.backCard) == 0:
            return
        elif len(self.cardFront) == 0:
            return
        elif len(self.backCard) == 0:
            return
        # If the user puts input in both boxes, the deck creation will be successful.
        else:
            for deck in mainController.get_decks():
